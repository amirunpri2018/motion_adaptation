...
def _load_frame(idx, im, an, imgs, flow_dir, flow_into_past, flow_into_future, flow_as_angle):
  ...
  im_val= scipy.misc.imresize(im_val, (480,854))
  scipy.misc.imsave('test.jpg', im_val)
  ...
  an_raw= scipy.misc.imresize(an_raw, (480,854), 'nearest')
  scipy.misc.imsave('testan.jpg', an_raw)
...
class Segtrackv2OneshotDataset(OneshotImageDataset):
  def __init__(self, config, subset, use_old_label):
    ...
    super(Segtrackv2OneshotDataset, self).__init__(config, NUM_CLASSES, VOID_LABEL, subset, image_size=DAVIS_IMAGE_SIZE,
  ...
  def set_video_idx(self, video_idx):
    super(Segtrackv2OneshotDataset, self).set_video_idx(video_idx)
  ...
  def load_videos(self, fn, data_dir, video_range=None):
    ...
    elif Segtrackv2OneshotDataset._video_data is not None:
      #use this cache only if not doing offline adaptation!
      return Segtrackv2OneshotDataset._video_data
    ...
    Segtrackv2OneshotDataset._video_data = (video_tags, videos)
    end = time.time()
    elapsed = end - start
